...
for /F "usebackq delims=" %%a in ("%MAVEN_PROJECTBASEDIR%\.mvn\wrapper\maven-wrapper.properties") do (
    set line=%%a
    if "!line:~0,25!"=="distributionUrl=https\://" (
        set DOWNLOAD_URL=!line:~16!
    )
    if "!line:~0,21!"=="wrapperUrl=https\://" (
        set WRAPPER_URL=!line:~11!
    )
)
@endlocal & set DOWNLOAD_URL=%DOWNLOAD_URL% & set WRAPPER_URL=%WRAPPER_URL%
...
IF EXIST %WRAPPER_JAR% (
    ...
    IF NOT "%DOWNLOAD_URL%"=="" (
        ...
        echo Error: WRAPPER_JAR not found and DOWNLOAD_URL not set. >&2
        goto error
    )
)
